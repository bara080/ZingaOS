...
// CRM · WhatsApp — a CONSENT-GATED WhatsApp channel (Meta WhatsApp Business
// Cloud API). Faithful copy of the SMS view, wired to Meta instead of Telnyx.
//
// LEGAL: WhatsApp Business messaging requires an explicit opt-in before a
// business messages a user, AND — for business-initiated messages outside the
// 24-hour customer-service window — an approved message template. This surface
// can ONLY message a number that has a non-opted-out row in the consent ledger
// (ops.whatsapp_consent). The send route hard-gates on
// operator_whatsapp_consent_is_allowed(); this UI mirrors that — the Send button
// is disabled for any number that is not opted_in, and STOP/opt-out is shown
// clearly and honored irreversibly-by-inbound. Layout mirrors SmsView:
...
import {
  ...
  useWhatsappConsent,
  useWhatsappSend,
  useWhatsappConsentAdd,
  useWhatsappConsentOptout,
  useWhatsappDraft,
  useLeads,
} from '../hooks';
import { leadName, type WhatsAppThread, type WhatsAppConsent, type Lead } from '../api';
...
export function WhatsAppView() {
  const consentQ = useWhatsappConsent(true);
  const threadsQ = useWhatsappThreads(true);
  ...
  const threadByPhone = useMemo(() => {
    const m = new Map<string, WhatsAppThread>();
  ...
  const threadQ = useWhatsappThread(selected, true);
  const messages = threadQ.data?.messages ?? [];

  const send = useWhatsappSend();
  const draft = useWhatsappDraft();
  const optout = useWhatsappConsentOptout();
  ...
  return (
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        <h2 style={{ fontFamily: C.sans, fontSize: 18, fontWeight: 600, color: C.ink, margin: 0 }}>WhatsApp</h2>
        <span
          style={{
            display: 'inline-flex',
            alignItems: 'center',
        ...
        >
          ...
          {configured ? 'WhatsApp connected' : 'WhatsApp not configured'}
        </span>
      </div>
      ...
      >
        ...
        <span>
          <b style={{ color: C.ink }}>Consent-gated.</b> Only numbers with a recorded WhatsApp opt-in can
          be messaged — scraped-only leads are never messaged. Meta requires an approved message template
          for business-initiated messages outside the 24-hour window. STOP / UNSUBSCRIBE is honored
          automatically and can&apos;t be undone by an inbound reply.
        </span>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr 260px', gap: 14, alignItems: 'start' }}>
            ...
            <div style={{ ...colCard, display: 'flex', flexDirection: 'column', minHeight: 460, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingBottom: 10, borderBottom: `1px solid ${C.line}`, minWidth: 0 }}>
                ...
                {active && <div style={{ fontFamily: C.mono, fontSize: 10.5, color: C.ink3 }}>{active.phone} · WhatsApp</div>}
                <span style={{ flex: 1 }} />
              ...
              <div style={{ marginTop: 10, borderTop: `1px solid ${C.line}`, paddingTop: 10 }}>
                {active && !isAllowed && (
                  <div style={{ fontFamily: C.mono, fontSize: 10.5, color: C.red, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Ban size={12} /> This number is opted-out. Messaging is blocked.
                  </div>
                )}
                <textarea
                  value={reply}
              ...
              {!active ? (
                ...
                <>
                  ...
                  <div style={{ fontFamily: C.mono, fontSize: 9.5, color: C.ink3, marginTop: 14, lineHeight: 1.6 }}>
                    Messages send from the WhatsApp Business number via the Meta Cloud API — consent-gated,
...
function AddConsentForm() {
  const add = useWhatsappConsentAdd();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7, border: `1px solid ${C.line}`, borderRadius: 10, padding: 11, background: C.panel2 }}>
      ...
      <div style={{ fontFamily: C.mono, fontSize: 9.5, color: C.ink3, lineHeight: 1.5 }}>
        Only record a real, documented opt-in. This authorizes messaging this number.
      </div>
...
function DormantPanel({ configured, optedInCount }: { configured: boolean; optedInCount: number }) {
  return (
    ...
    >
      <MessageCircle size={20} color={C.amber} style={{ flexShrink: 0, marginTop: 2 }} />
      <div>
        <div style={{ fontFamily: C.sans, fontSize: 13.5, fontWeight: 600, color: C.ink, marginBottom: 6 }}>
          WhatsApp channel is ready, but dormant
        </div>
        <div style={{ fontFamily: C.mono, fontSize: 11, color: C.ink2, lineHeight: 1.7 }}>
          ...
          <ul style={{ margin: '8px 0 0', paddingLeft: 18 }}>
            <li style={{ color: configured ? C.green : C.amber }}>
              {configured ? '✓' : '•'} A WhatsApp Business sender + Meta Cloud API creds (WHATSAPP_TOKEN and
...
// ── small shared bits ───────────────────────────────────────────────────────
function StatusPill({ status }: { status: WhatsAppConsent['status'] | 'unknown' }) {
