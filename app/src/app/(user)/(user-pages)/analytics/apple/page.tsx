...
import type { AppStoreSummary, AppStoreReviewsResponse } from '@/features/apple/types';
...
async function fetchSummary(): Promise<AppStoreSummary> {
  const res = await fetch('/api/trackers/apple/summary');
  if (!res.ok) throw new Error('Failed to fetch Apple summary');
  return res.json();
}

async function fetchReviews(params: { page: number; rating: string }): Promise<AppStoreReviewsResponse> {
  const query = new URLSearchParams({ page: String(params.page) });
  if (params.rating !== 'all') query.set('rating', params.rating);
  const res = await fetch(`/api/trackers/apple/reviews?${query}`);
  if (!res.ok) throw new Error('Failed to fetch Apple reviews');
  return res.json();
}
...
export default function AppleStorePage() {
  ...
  const summary = useQuery<AppStoreSummary>({
    queryKey: ['apple-summary'],
    queryFn: fetchSummary,
    staleTime: 5 * 60 * 1000,
  });

  const reviews = useQuery<AppStoreReviewsResponse>({
    queryKey: ['apple-reviews', page, ratingFilter],
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Apple App Store</h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  ...
                  <th scope="col" className="text-left px-4 py-2 text-xs text-muted-foreground font-medium hidden md:table-cell">User</th>
                  <th scope="col" className="text-left px-4 py-2 text-xs text-muted-foreground font-medium hidden lg:table-cell">Territory</th>
                ...
                {reviews.data?.data.map((review) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 max-w-xs">
                      {review.title && (
                        <p className="font-medium text-foreground text-xs leading-snug">{review.title}</p>
                      )}
                      <p className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{review.body}</p>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <p className="text-xs text-foreground">{review.userName || '—'}</p>
                    </td>
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <p className="text-xs text-muted-foreground">{review.territory || '—'}</p>
                    </td>
