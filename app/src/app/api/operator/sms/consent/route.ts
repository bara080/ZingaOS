...
import { telnyxConfigured } from '@/lib/operator/telnyx';

// /api/operator/sms/consent
...
// opt-in here asserts that TCPA prior express consent was actually captured
...
export async function GET() {
  ...
  const { data, error } = await admin.rpc('operator_sms_consent_list', { p_limit: 300 });
  if (error) {
    console.error('operator/sms/consent list RPC error', error.message);
    return NextResponse.json({ error: 'Failed to load consent list' }, { status: 500 });
  }
  return NextResponse.json({ consent: data ?? [], configured: telnyxConfigured() });
}
...
export async function POST(req: Request) {
  ...
  try {
    const { data, error } = await admin.rpc('operator_sms_consent_add', {
      p_phone: phone,
      p_name: name || null,
      p_lead_id: leadId,
      p_source: source,
