...
// POST /api/operator/whatsapp/draft  { name?, business?, category?, borough?, base?, instruction? }
// Optional AI helper for the WhatsApp composer. Generates (or rewrites `base`) a
// SHORT, compliant WhatsApp reply in Zinga's voice — 1–2 sentences, under ~300
// chars, no links unless natural, and ALWAYS ends with "Reply STOP to opt out".
// Falls back to a deterministic template. Draft only — the operator still
// reviews + sends, and the send route hard-gates on consent regardless.
...
export async function POST(req: Request) {
  ...
  const RULES =
    'Write ONE short WhatsApp message (max ~300 characters, ideally 1–2 sentences). This is a ' +
    'consented WhatsApp Business message, so keep it plain, human, and non-spammy. Do NOT use ' +
  ...
  try {
    const { text, provider } = await generate(`${ZINGA_VOICE}\n\n${RULES}`, ctx || 'Write a friendly first WhatsApp message.', 160);
